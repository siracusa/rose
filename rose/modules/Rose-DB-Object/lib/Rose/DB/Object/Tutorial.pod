...
Finally, although most of the examples directly modify the C<@ISA> package variable in order to set up inheritance, the L<use base|base> directive works just as well.  In some situations, one may be preferable to the other (L<use base|base> is run at compile-time whereas the C<@ISA> modifications happen at run-time) but in the common case it shouldn't matter which you choose.
...
To get up to speed quickly with L<Rose::DB>, read the L<Rose::DB::Tutorial> documentation.  The rest of this tutorial will assume the existence of a C<My::DB> class created as L<described|Rose::DB::Tutorial/"Multiple data sources using namespaces"> in the L<Rose::DB tutorial|Rose::DB::Tutorial>.  Here's a simple incarnation of C<My::DB>.
...
Operations 2 through 6 are done through the L<metadata object|Rose::DB::Object::Metadata> associated with this class.  The table must have a primary key, and may have zero or more unique keys.  The primary key and each unique key may contain multiple columns.
...
Now C<Product> will create a L<My::DB> object when it needs to connect to the database.  Note that this means that each C<Product> object will have its C<My::DB> object.  See the section, L<"A brief digression: database objects"> for an explanation and alternatives.
...
(Again, note that all C<My::DB::Object>-derived objects will get their own C<My::DB> objects given this definition of C<init_db()>.  See the section, L<"A brief digression: database objects"> for more information.)
...
The Perl code for the C<Product::Manager> class shown above can be generated automatically with the L<perl_manager_class|Rose::DB::Object::Metadata/perl_manager_class> method.  The L<make_manager_class|Rose::DB::Object::Metadata/make_manager_class> method will both generate the code and L<eval|perlfunc/eval>uate it for you, automating the entire process of creating a manager class from within your L<Rose::DB::Object>-derived class.
...
The return value is a reference to an array of C<Product> objects in the example above.  Now let's go to the other extreme.
...
Counting objects is straightforward.  The C<get_products_count()> method takes the same same kinds of arguments as C<get_products()> and C<get_products_iterator()>.  (Noticing a pattern here?)  It returns the count.
...
See the L<Rose::DB> documentation to explore the capabilities of these db objects.  Most of the time, you won't have to be concerned about them.  But it is sometime important to ensure that all objects share the same L<db|Rose::DB::Object/db>.

If the L<db|Rose::DB::Object/db> attribute doesn't exist, it is created by calling L<init_db()|Rose::DB::Object/init_db>.  The typical C<init_db()> method simply builds a new database object and returns it.

    package Product;
    ...
    sub init_db { My::DB->new }

This means that each C<Product> object will have its C<My::DB> object and therefore (in the absence of modules like L<Apache::DBI>) its own connection to the database.  If this not what you want, you can make C<init_db()> return the same C<My::DB> object to every C<Product> object.  I don't recommend this approach since it's harder to ensure that the database handle will be closed when all C<Product> objects go out of scope, but that may not be important for your application.

Since C<init_db()> is only called if a C<Product> object does not already have a L<db|Rose::DB::Object/db> object, one way to share a single C<My::DB> object amongst several C<Product> objects is to do so explicitly, either by pre-creating the C<My::DB> object:
...
One final note for L<mod_perl> users: when using L<Apache::DBI>, even if each C<Product> has its own C<My::DB> object, remember that they will all share a single underlying L<DBI> database handle.
...
Cross-database migration is another important use for explicitly shared C<db> objects.  Here's how to move a product from a production database to an archive database.
...
Second, note the default value for the C<date_created> column.  It's a string containing a call to the PL/SQL function C<now()>, which can actually only be run within the database.  That's okay, because L<Rose::DB::Object> will pass that value through to the database rather than trying to execute it itself.
...
In some circumstances, L<Rose::DB::Object> can even "fake" a data type for use with a database that does not natively support it.  For example, the L<array|Rose::DB::Object::Metadata::Column::Array> column type is natively supported by Postgres, but it will also work with MySQL using a VARCHAR column as a stand-in.
...
First, notice that the C<Vendor> class is loaded with a C<use> statement near the top of the class definition.  This is important.  A foreign key cannot be defined unless the L<Rose::DB::Object>-derived class that fronts the foreign table is also loaded.  The auto-initialization process will try to load such classes for you, but the "manual" approach shown above will not.  As usual, I prefer the more explicit approach.
...
Passing a reference to an empty array will remove all colors associated with a particular product by deleting all its mapping table entries.
...
In the Perl code below, I've switched to using the L<base> module to specify the base class inheritance.  I've done this for the sake of brevity, but also to show that it works just as well as setting C<@ISA> directly.
...
The output is shown below.  Compare it to the manually created Perl code L<shown above|/"Relationship code summary"> and you'll see that it's nearly identical.  Again, careful table name choices really help here.  Do what the L<convention manager|Rose::DB::Object::ConventionManager> expects (or write your own convention manager subclass that does what I<you> expect) and automation like this can work very well.

  package My::Color;

  use strict;

  use My::DB::Object::Base1;
  our @ISA = qw(My::DB::Object::Base1);
  ...
  __PACKAGE__->meta->primary_key_columns([ 'id' ]);
  __PACKAGE__->meta->add_unique_keys([ 'product_id', 'region' ]);
  ...
  __PACKAGE__->meta->table('product_colors');
...
The resulting SQL is a bit more complex.
    ...
    FROM
      ...
      LEFT JOIN colors t3 ON(t2.color_id = t3.id)
    WHERE
      t1.id > 15 AND
      t1.name LIKE 'Kite%'
...
The SQL would now look something like this.
    ...
    WHERE
      t1.id > 15 AND
      t1.name LIKE 'Kite%' AND
      t1.vendor_id = t5.id AND
      t5.region_id = t6.id AND
      t6.name = 'UK'
    ORDER BY t1.name
...
http://rose.sourceforge.net/wiki/index.php/RDBO/MailingList/Extending
