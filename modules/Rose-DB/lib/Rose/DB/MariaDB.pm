...
{
  local $@;
  eval { require DBD::MariaDB }; # Ignore errors
}
...
sub mariadb_auto_reconnect     { shift->dbh_attribute_boolean('mariadb_auto_reconnect', @_) }
sub mariadb_client_found_rows  { shift->dbh_attribute_boolean('mariadb_client_found_rows', @_) }
sub mariadb_compression        { shift->dbh_attribute_boolean('mariadb_compression', @_) }
sub mariadb_connect_timeout    { shift->dbh_attribute_boolean('mariadb_connect_timeout', @_) }
sub mariadb_embedded_groups    { shift->dbh_attribute('mariadb_embedded_groups', @_) }
sub mariadb_embedded_options   { shift->dbh_attribute('mariadb_embedded_options', @_) }
sub mariadb_local_infile       { shift->dbh_attribute('mariadb_local_infile', @_) }
sub mariadb_multi_statements   { shift->dbh_attribute_boolean('mariadb_multi_statements', @_) }
sub mariadb_read_default_file  { shift->dbh_attribute('mariadb_read_default_file', @_) }
sub mariadb_read_default_group { shift->dbh_attribute('mariadb_read_default_group', @_) }
sub mariadb_socket             { shift->dbh_attribute('mariadb_socket', @_) }
sub mariadb_ssl                { shift->dbh_attribute_boolean('mariadb_ssl', @_) }
sub mariadb_ssl_ca_file        { shift->dbh_attribute('mariadb_ssl_ca_file', @_) }
sub mariadb_ssl_ca_path        { shift->dbh_attribute('mariadb_ssl_ca_path', @_) }
sub mariadb_ssl_cipher         { shift->dbh_attribute('mariadb_ssl_cipher', @_) }
sub mariadb_ssl_client_cert    { shift->dbh_attribute('mariadb_ssl_client_cert', @_) }
sub mariadb_ssl_client_key     { shift->dbh_attribute('mariadb_ssl_client_key', @_) }
sub mariadb_use_result         { shift->dbh_attribute_boolean('mariadb_use_result', @_) }
sub mariadb_bind_type_guessing { shift->dbh_attribute_boolean('mariadb_bind_type_guessing', @_) }

sub mariadb_enable_utf8
{
  my($self) = shift;
  $self->dbh->do('SET NAMES utf8')  if(@_ && $self->has_dbh);
  $self->dbh_attribute_boolean('mariadb_enable_utf8', @_)
}

sub mariadb_enable_utf8mb4
{
  my($self) = shift;
  $self->dbh->do('SET NAMES utf8mb4')  if(@_ && $self->has_dbh);
  $self->dbh_attribute_boolean('mariadb_enable_utf8mb4', @_)
}
...
sub insertid_param { 'mariadb_insertid' }

sub last_insertid_from_sth { $_[1]->{'mariadb_insertid'} }
...
sub refine_dbi_column_info
{
  ...
  if($col_info->{'TYPE_NAME'} eq 'set')
  {
    ...
    $col_info->{'RDBO_SET_VALUES'} = $col_info->{'mariadb_values'};
  }
  elsif($col_info->{'TYPE_NAME'} eq 'enum')
  {
    $col_info->{'RDBO_ENUM_VALUES'} = $col_info->{'mariadb_values'};
  }

  # Consider (big)int autoincrement to be (big)serial
  if($col_info->{'mariadb_is_auto_increment'} &&
