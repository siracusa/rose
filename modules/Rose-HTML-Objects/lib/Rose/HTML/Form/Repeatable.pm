...
Rose::HTML::Form::Repeatable - Repeatable sub-form wrapper class.
...
A repeatable form decides how many of each repeated sub-form it should contain based on the contents of the query parameters contained in the L<params|Rose::HTML::Form/params> attribute.  If there are no L<params|Rose::HTML::Form/params>, then the L<default_count|/default_count> determines the number of repeated forms.
...
Given an integer argument greater than zero, creates, adds to the form, and returns a new numbered L<prototype_form_clone|/prototype_form_clone> sub-form.
...
Get or set the name of the L<Rose::HTML::Form>-derived class used by the L<prototype_form_clone|/prototype_form_clone> to create each repeated sub-form.  The default value is determined by the L<default_form_class|/default_form_class> class method.
...
=item B<prototype_form_clone [FORM]>
