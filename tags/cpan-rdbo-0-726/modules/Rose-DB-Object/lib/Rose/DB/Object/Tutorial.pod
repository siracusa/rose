...
Although most of the examples in this tutorial use the L<base.pm|base> module to set up inheritance, directly modifying the C<@ISA> package variable usually works just as well.  In situations where there are circular relationships between classes, the C<use base ...> form is preferable because it runs at compile-time, whereas C<@ISA> modification happens at run-time.

Finally, be sure to set up inheritance before loading any related modules.
