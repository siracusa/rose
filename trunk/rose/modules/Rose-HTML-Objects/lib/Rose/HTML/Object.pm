...
C<validate_html_attrs()> method.   Subclasses can set this attribute during
object construction to make the effect class-wide.  (You will also want to
look at the C<autoload_html_attr_methods()> class attribute.)
...
using the C<required_html_attr_value()> method or by passing the DEFAULT
...
attribute using the C<required_html_attr_value()> method or by passing a
...
methods, opting instead to use C<html_attr()> everywhere (and you can set
...
C<html_attrs_string()> and C<xhtml_attrs_string()> even if they have been
deleted by C<delete_html_attr()> or one of its variants.  If a required HTML
...
of the strings returned by C<html_element()> and C<html_attrs_string()>,
...
A synonym for C<xhtml_tag()>.

=item B<xhtml_tag>

Serializes the object as an XHTML tag.  In other words, it is the
concatenation of the strings returned by C<xhtml_element()> and
C<xhtml_attrs_string()>, wrapped with the appropriate angled brackets and
