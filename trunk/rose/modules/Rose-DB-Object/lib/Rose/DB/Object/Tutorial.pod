...
We've already seen that "one to one" and "many to one" relationships can be represented by foreign keys in the database, but that's not a requirement.  It's perfectly possible to have either of those two kinds of relationships in, for example, a database that has no native support for foreign keys.  MySQL using the MyISAM data storage format is a common example.
...
The C<price> column will be removed from the C<products> table now.
...
=item * A list or reference to an array of primary key value (but only if the foreign table has a single-column primary key).
...
Deleting a product now becomes slightly more interesting.  The naive approach fails.
    ...
    # "prices"
    # DETAIL:  Key (id)=(89) is still referenced from table "prices".
