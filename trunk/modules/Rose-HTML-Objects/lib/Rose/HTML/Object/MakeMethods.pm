...
sub array
{
  ...
  if($interface eq 'get_set_init')
  {
    ...
    $methods{$name} = sub
    {
      ...
      {
        $self->{$key} = [ map { _coerce_html_object($_) } @{$_[0]} ];
      }
      else
      {
        $self->{$key} = [ map { _coerce_html_object($_) } @_ ];
      }
  ...
  elsif($interface eq 'get_set_item')
  {
    $methods{$name} = sub
    {
      ...
      if(@_ == 2)
      {
        return $self->{$key}[$_[0]] = _coerce_html_object($_[1]);
      }
      else
      {
        return $self->{$key}[$_[0]]
  ...
  elsif($interface eq 'unshift')
  {
    $methods{$name} = sub
    {
      ...
      unshift(@{$self->{$key}}, map { _coerce_html_object($_) } (@_ == 1 && ref $_[0] eq 'ARRAY') ? @{$_[0]} : @_);
    }
  }
  ...
  elsif($interface =~ /^(?:push|add)$/)
  {
    $methods{$name} = sub
    {
      my($self) = shift;
      Carp::croak "Missing value(s) to add"  unless(@_);
      push(@{$self->{$key}}, map { _coerce_html_object($_) } (@_ == 1 && ref $_[0] && ref $_[0] eq 'ARRAY') ? @{$_[0]} : @_);
    }
  }
...
sub _coerce_html_object
{
  ...
  if(!ref $arg)
  {
    return Rose::HTML::Text->new(text => $arg);
  }
