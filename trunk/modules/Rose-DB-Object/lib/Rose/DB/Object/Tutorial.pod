...
Although most of the examples use the L<base> module to set up inheritance, directly modifying the C<@ISA> package variable usually works just as well.  In situations where there are circular relationships between classes, however, the C<use base ...> form is preferable because it runs at compile-time, whereas the C<@ISA> modifications happen at run-time.
