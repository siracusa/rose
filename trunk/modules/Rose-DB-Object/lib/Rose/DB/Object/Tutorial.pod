...
Note how the multi-column unique key is specified as reference to an array containing another reference to an array of column names.  Removing the "wrapper" array reference would cause it to be interpreted as two single-column unique keys instead of one two-column unique key.
