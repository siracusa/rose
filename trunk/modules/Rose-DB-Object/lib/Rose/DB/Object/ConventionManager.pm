...
Given the name of a foreign class, any pre-existing foreign key name, a reference to a hash of L<key columns|Rose::DB::Object::Metadata::ForeignKey/key_columns>, and a reference to a hash whose keys are foreign key names used in this class already, return a L<name|Rose::DB::Object::Metadata::ForeignKey/name>  for the foreign key.
...
If there is just one pair of columns in KEY_COLUMNS, and if the name of the local column ends with an optional underscore and the name of the referenced column, then that part of the column name is removed and the remaining string is used as the foreign key name.  For example, given the following tables:
